  ...
  return (
    <div className="max-sm:py-3 notidiv absolute bg-white border-1 rounded-md shadow-2xl  w-80 h-72 max-sm:w-screen max-sm:h-screen top-[3.5rem] max-sm:top-0 right-28 max-sm:right-0 sm:z-30 max-sm:z-30">
      ...
      {enableNoti ? (
          ...
          <div className="notification content p-1 m-1 rounded-b-lg h-[80%] overflow-y-auto overflow-x-hidden ">
            {notiStatus === "loading" ? (
              ...
              notiItem.map((item) => (
                <div
                  key={item.id}
                  className="content-item m-1 p-1 flex items-center justify-between"
                >
                  {item.read === !true ? (
                    <div className="p-0.5">
        ...
        <p className="text-center text-gray-500 hover:text-blue-500 cursor-pointer">Notifications are disabled</p>
      )}
    </div>
  );
};
